...
def get_temporal_importance_table_interpret_output(
    ...
    """
    Temporal importance berbasis attention decoder->encoder dari TFT (via interpret_output).

    Output:
      DataFrame dengan kolom:
        - Date (history window / encoder steps)
        - Temporal Importance (%)

    Catatan:
      - Bentuk attention dan API predict(mode="raw") bisa beda antar versi pytorch-forecasting.
      - Implementasi ini mencoba beberapa fallback agar tetap jalan.
    """
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            # try to get raw predictions with x
            raw_out = None
            x_out = None

            try:
                pred = model.predict(loader if False else batch, mode="raw", return_x=True)  # may fail
                # Some versions allow passing batch; others require dataloader. We'll handle below.
                raw_out, x_out = pred, None
            except Exception:
                pass

            if raw_out is None:
                # Preferred path: call on full dataloader for compatibility
                try:
                    pred = model.predict(loader, mode="raw", return_x=True)
                    # pred might be Prediction object, tuple, or dict-like
                    raw_out = pred
                except Exception:
            ...
            # Extract raw prediction tensor for interpret_output
            # Cases:
            # 1) Prediction object: has .output and maybe .x
            # 2) tuple: (raw, x) or (raw, index, x)
            # 3) dict output from forward already
            raw_pred = None
            x_pack = None

            if hasattr(raw_out, "output"):
                raw_pred = raw_out.output
                x_pack = getattr(raw_out, "x", None)
            elif isinstance(raw_out, (list, tuple)):
                # try pick first tensor-like / dict-like as raw, last as x if dict
                if len(raw_out) >= 1:
                    raw_pred = raw_out[0]
                if len(raw_out) >= 2 and isinstance(raw_out[-1], dict):
                    x_pack = raw_out[-1]
            ...
            # Interpret
            try:
                interpretation = model.interpret_output(raw_pred, reduction="sum")
            except Exception:
                # some versions expect raw_pred.output
                try:
                    interpretation = model.interpret_output(getattr(raw_pred, "output", raw_pred), reduction="sum")
                except Exception as e:
                    raise RuntimeError(f"Gagal interpret_output: {e}")

            att = interpretation.get("attention", None)
            if att is None:
                continue

            # attention tensor usually [B, dec_len, enc_len]
            att_tensor = None
            if torch.is_tensor(att):
                att_tensor = att
            elif isinstance(att, dict):
                # try common keys, else first tensor value
                for k in ["attention", "decoder", "encoder_decoder", "weights"]:
                    if k in att and torch.is_tensor(att[k]):
                        att_tensor = att[k]
                        break
            ...
            if enc_sum is None:
                ...
                m = min(len(enc_sum), len(enc_scores))
                enc_sum[:m] += enc_scores[:m]

            # Important: if we called predict on full loader, break to avoid double counting
            # (because it returns all batches). But in that path, we only need once.
            if not isinstance(raw_out, dict) and hasattr(raw_out, "output"):
                # This is likely aggregated already; stop
                break

    if enc_sum is None:
        enc_len = int(params.get("max_encoder_length", 60))
        enc_sum = np.ones(enc_len, dtype=float)

    s = float(np.sum(enc_sum))
    enc_imp = enc_sum / s if s > 0 else enc_sum
    ...
    temp["Temporal Importance (%)"] = (temp["Temporal Importance"] * 100).round(2)
    return temp



# =========================
# Streamlit UI (Upgraded UI/UX + Temporal Importance)
# =========================
st.set_page_config(page_title="BBCA TFT Forecast", layout="wide", page_icon="📈")
...
st.markdown(
    ...
    <div class="card">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap;">
        <div>
          <h2 style="margin:0;">📈 {TICKER} TFT Forecast Dashboard</h2>
          <div class="muted">Realtime price (Yahoo) + Fundamentals (Quarterly→Daily) + Technicals → Forecast dengan Temporal Fusion Transformer</div>
          <div style="margin-top:10px;">
            <span class="badge">Realtime Yahoo</span>
            <span class="badge">Fundamental QoQ/YoY</span>
            <span class="badge">TFT Explainability</span>
          </div>
        </div>
        <div class="muted" style="text-align:right;">
          <div>Group ID: <b>BBCA</b></div>
          <div>Target: <b>ret_log</b></div>
        </div>
      </div>
    </div>
    """,
...
with tab_explain:
    if st.session_state.pred_df is None:
        ...
        with sub_time:
            ...
            enc_window = st.slider("Window history untuk interpretasi (baris terakhir)", 120, 800, 320, 20)
            top_dates = st.slider("Top-N tanggal paling crucial", 5, 60, 15)

            if st.session_state.temporal_importance is None:
                with st.spinner("Menghitung temporal importance (attention)…"):
                    try:
                        df_interp = df_model.tail(enc_window).copy()
                        df_interp = align_df_to_dataset_params(df_interp, params)
                        temp = get_temporal_importance_table_interpret_output(model, params, df_interp, n_batches=3)
                        st.session_state.temporal_importance = temp.copy()
                    except Exception as e:
                        st.error("Gagal menghitung temporal importance.")
                        st.exception(e)

            if st.session_state.temporal_importance is not None:
                temp = st.session_state.temporal_importance.copy()

                # Chart
                st.subheader("Attention over Time (Encoder Window)")
                if _HAS_PLOTLY:
                    figt = go.Figure()
                    figt.add_trace(go.Scatter(x=temp["Date"], y=temp["Temporal Importance (%)"], mode="lines", name="Temporal Importance (%)"))
                    figt.update_layout(height=360, margin=dict(l=10, r=10, t=30, b=10), xaxis_title="Date (history)", yaxis_title="Importance (%)")
                ...
                st.download_button(
                    "⬇️ Download temporal importance CSV",
                    data=temp.to_csv(index=False).encode("utf-8"),
                    file_name="temporal_importance_attention.csv",
                    mime="text/csv",
