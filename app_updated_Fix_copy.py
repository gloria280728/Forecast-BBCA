...
def drop_nan_target_rows(df: pd.DataFrame, target: str) -> pd.DataFrame:
    """
    TFT TIDAK mengizinkan NaN pada target.
    Drop baris historis yang targetnya NaN (biasanya awal series).
    """
...
def align_df_to_dataset_params(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    """
    Align dataframe ke dataset params TANPA mengisi target (ret_log) di future.
    Known future -> boleh ffill/bfill
    Unknown future & target -> BIARKAN NaN
    """
    ...
    # ✅ FILL HANYA KNOWN FUTURE
    known_cols = set(params.get("time_varying_known_reals", []))
    known_cols |= set(params.get("time_varying_known_categoricals", []))
    known_cols |= {"time_idx", "group_id"}

    fill_cols = [c for c in known_cols if c in df.columns]
    df[fill_cols] = df[fill_cols].ffill().bfill()

    # ❗ JANGAN fill target
    if "ret_log" in df.columns:
        df.loc[df["time_idx"] > df["time_idx"].iloc[-1] - 1, "ret_log"] = np.nan
...
def predict_direct(model, params, df_all: pd.DataFrame) -> np.ndarray:
    ...
    df_all = drop_nan_target_rows(df_all, params.get("target", "ret_log"))
...
with st.sidebar:
    ...
    with st.form("controls_form", border=False):
        ...
        horizon_ui = st.slider("Forecast horizon (trading days)", 1, 20, 20)
        horizon = int(params.get("max_prediction_length", horizon_ui))
...
# ---------- Load price ----------
try:
    price = load_bbca_price_from_yahoo(period="5y", interval="1d")
except Exception as e:
...
if isinstance(view_range, tuple) and len(view_range) == 2:
    ...
    view_end = pd.to_datetime(view_range[1])+ timedelta(days=1)
else:
...
with tab_overview:
    ...
    with c1:
        st.subheader("Historical Close (with Trend)")
        with c1:
            show_ema = st.checkbox(
                "Show EMA (trend)",
                value=True,
                help="EMA menunjukkan arah tren jangka pendek harga"
            )
        if _HAS_PLOTLY:
            fig = go.Figure()
            ...
            fig.update_layout(
                height=420,
                margin=dict(l=10, r=10, t=30, b=40),  # b dikit diperbesar
                xaxis_title="Date",
                yaxis_title="Price",
    ...
    with c1:    
        ...
        st.subheader("Market Pressure (RSI)")
    ...
    with c2:
        ...
        ema = latest["EMA_20"]
        rsi = latest["RSI_14"]
        ema_gap = (price / ema - 1) * 100

        # ===== STATUS LOGIC =====
        if price > ema:
            trend_label = "UPTREND"
            trend_color = "#22c55e"   # green
            trend_desc = "Harga berada DI ATAS tren rata-rata"
        else:
            trend_label = "DOWNTREND"
            trend_color = "#ef4444"   # red
            trend_desc = "Harga berada DI BAWAH tren rata-rata"

        if rsi >= 60:
            pressure_label = "BUY PRESSURE"
            pressure_color = "#22c55e"
            pressure_desc = "Tekanan beli relatif tinggi"
        elif rsi <= 40:
            pressure_label = "SELL PRESSURE"
            pressure_color = "#ef4444"
            pressure_desc = "Tekanan jual relatif tinggi"
        else:
            pressure_label = "NEUTRAL"
            pressure_color = "#eab308"  # yellow
            pressure_desc = "Tekanan pasar relatif seimbang"
...
<div style="font-size:12px; letter-spacing:0.22em; text-transform:uppercase; color:rgba(229,231,235,0.6);">
Market Snapshot (Latest)
</div>

<div style="margin-top:14px;">
  <div style="font-size:34px; font-weight:800; color:{trend_color};">
    {trend_label}
  </div>
  <div style="font-size:14px; color:rgba(229,231,235,0.8);">
    {trend_desc}
  </div>
</div>
...
<div style="display:flex; justify-content:space-between;">
  <div>
    <div style="font-size:12px; color:rgba(229,231,235,0.6);">LAST PRICE</div>
    <div style="font-size:22px; font-weight:700;">{price:,.0f}</div>
  </div>

  <div>
    <div style="font-size:12px; color:rgba(229,231,235,0.6);">VS EMA 20</div>
  ...
  <div>
    <div style="font-size:12px; color:rgba(229,231,235,0.6);">RSI (14)</div>
    <div style="font-size:22px; font-weight:700; color:{pressure_color};">
      {rsi:.1f}
    </div>
  </div>
</div>
...
""")
        ...
        <b>Indicator Color Guide</b><br>
        <span style="color:#22c55e; font-weight:600;">🟢 Hijau</span> → Kondisi positif (tren naik / tekanan beli dominan)<br>
        <span style="color:#eab308; font-weight:600;">🟡 Kuning</span> → Netral (belum ada arah kuat)<br>
        <span style="color:#ef4444; font-weight:600;">🔴 Merah</span> → Kondisi lemah (tren turun / tekanan jual dominan)<br>

        <div style="margin-top:6px; font-size:12px; color:rgba(229,231,235,0.6);">
        Catatan: Warna EMA menunjukkan <b>arah tren harga</b>, sedangkan warna RSI menunjukkan <b>tekanan beli/jual</b>.
        </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab_forecast:
    ...
    st.subheader("Forecast Close (future with Trend)")

    # ==============================
    # Build extended series for EMA
    # ==============================

    HIST_DAYS = 22

    df_hist_short = df_model.tail(HIST_DAYS)[["Date", "Close"]].copy()
    df_hist_short = df_hist_short.rename(columns={"Close": "Price"})

    df_forecast_price = pred_df[["Date", "Close_pred"]].copy()
    df_forecast_price = df_forecast_price.rename(columns={"Close_pred": "Price"})

    df_price_ext = pd.concat(
        [df_hist_short, df_forecast_price],
        ignore_index=True
    )

    df_price_ext["EMA_20"] = df_price_ext["Price"].ewm(span=20, adjust=False).mean()
    ...
    if _HAS_PLOTLY:
        ...
        show_ema_forecast = st.checkbox(
            "Show EMA 20 (trend)",
            value=True
        )

        if show_ema_forecast:
            fig2.add_trace(go.Scatter(
                x=df_price_ext["Date"],
                y=df_price_ext["EMA_20"],
                mode="lines",
                name="EMA 20 (Projected)",
                line=dict(dash="dot", color="#025B81", width=2),
                opacity=0.9
            ))

        # --- historical close ---
        fig2.add_trace(go.Scatter(
            x=df_hist_short["Date"],
            y=df_hist_short["Price"],
            mode="lines+markers",
            name="Historical Close (1M)",
        ...
        # fig2.add_trace(go.Scatter(
        #     x=df_hist_short["Date"],
        #     y=df_hist_short["EMA_20"],
        #     mode="lines",
        #     name="EMA 20 (Historical)",
        #     line=dict(dash="dot", color="")
        # ))
    ...
    if st.session_state.pred_df is None:
        ...
        with sub_time:
            ...
            if temp is None or temp.empty:
                ...
                st.caption(f"Ditampilkan: **{len(temp)}** tanggal terakhir")
