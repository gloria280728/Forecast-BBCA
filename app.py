...
# Streamlit UI
# =========================
st.set_page_config(page_title="BBCA TFT Forecast", layout="wide")
st.title("📈 BBCA TFT Forecast (Realtime Yahoo + Fundamentals)")
...
with st.sidebar:
    ...
    # optional: forecast mulai dari end view range atau latest
    ...
    st.caption("File yang harus ada: tft_model.ckpt, tft_dataset_params.pkl, fundamentals CSV")
# Load model + params
try:
...
# ✅ align ke trading dates (price dates) pake ffill (step function per quarter)
price_dates = pd.to_datetime(price["Date"]).sort_values()

fund_daily = (
    fund_q.set_index("Date")
          .sort_index()
          .reindex(price_dates, method="ffill")  # pakai fundamental terakhir yg available
          .bfill()                               # isi awal biar gak NaN
          .reset_index()
          .rename(columns={"index": "Date"})
)

# optional debug (hapus kalau udah beres)
st.write("CAR NA ratio (fund_daily):", float(fund_daily["CAR/KPMM (%)"].isna().mean()))

df = build_df_tft(price, fund_daily)

# align columns (including any special cols that training expects)
df = align_df_to_dataset_params(df, params)

# --- view filter (cuma buat tampilan) ---
min_d = df["Date"].min()
max_d = df["Date"].max()
...
st.caption(
    f"Forecast anchor date: {pd.to_datetime(df_model['Date'].iloc[-1]).date()} "
    f"(source: {'view_end' if forecast_from_view_end else 'latest'})"
)
...
if st.button("Run Forecast", type="primary"):
    ...
    try:
        if pred_len >= horizon:
            ...
            pred_df = pd.DataFrame({"Date": future_dates, "ret_pred": ret_pred, "Close_pred": close_path})
                # simpan hasil biar gak hilang saat rerun
            # st.session_state.pred_df = pred_df.copy()
            # st.session_state.anchor_date = pd.to_datetime(df_model["Date"].iloc[-1]).date()
            # st.session_state.anchor_close = float(df_model["Close"].iloc[-1])
        else:
    ...
    # ==== TAMBAH KOLOM % UNTUNG/RUGI (DI SINI) ====
    anchor_close = float(df_model["Close"].iloc[-1])

    pred_df["% Untung/Rugi dari Historical Close"] = (pred_df["Close_pred"] / anchor_close - 1.0) * 100.0
    pred_df["% Naik/Turun vs Hari Sebelumnya"] = pred_df["Close_pred"].pct_change() * 100.0
    pred_df.loc[pred_df.index[0], "% Naik/Turun vs Hari Sebelumnya"] = pred_df["% Untung/Rugi dari Historical Close"].iloc[0]
    ...
    # ubah jadi string % untuk tampilan
    pred_df["% Untung/Rugi dari Historical Close"] = pred_df["% Untung/Rugi dari Historical Close"].apply(fmt_pct)
    pred_df["% Naik/Turun vs Hari Sebelumnya"] = pred_df["% Naik/Turun vs Hari Sebelumnya"].apply(fmt_pct)

    # ✅ simpan pred_df yang SUDAH lengkap kolom % (biar aman pas rerun)
    st.session_state.pred_df = pred_df.copy()
    st.session_state.anchor_date = pd.to_datetime(df_model["Date"].iloc[-1]).date()
    st.session_state.anchor_close = float(df_model["Close"].iloc[-1])

    st.success("Forecast Selesai. Scroll ke bawah untuk lihat hasilnya.")
    st.rerun()

    # st.subheader("Forecast Close (future)")
    # st.line_chart(pred_df.set_index("Date")["Close_pred"])
    ...
    # st.markdown("---")
    # st.subheader("Feature importance (TFT)")

    # top_k = st.slider("Top features", 5, 50, 20)

    # try:
    #     # pakai window terakhir df_model (lebih cepat + relevan)
    #     # ambil sebagian akhir data biar interpretasi ga berat
    #     df_interp = df_model.tail(300).copy()
    #     df_interp = align_df_to_dataset_params(df_interp, params)

    #     fi = get_feature_importance_table(model, params, df_interp, top_k=top_k)
    #     st.session_state.feature_importance = fi.copy()
    #     st.dataframe(fi, use_container_width=True)

    #     st.caption("Angka lebih besar = feature lebih sering dipakai (berdasarkan variable selection weights).")
    # except Exception as e:
    ...
    #     mime="text/csv",
    #     key="download_forecast_inside",
    # )

# =========================
# Render results from session_state (persist across reruns)
# =========================
if st.session_state.pred_df is not None:
    pred_df = st.session_state.pred_df.copy()

    # kalau pred_df yang kesimpen belum ada kolom %, bikin ulang
    if ("% Untung/Rugi dari Historical Close" not in pred_df.columns) or ("% Naik/Turun vs Hari Sebelumnya" not in pred_df.columns):
        if st.session_state.anchor_close is not None:
            pred_df = add_profit_columns(pred_df, float(st.session_state.anchor_close))
            st.session_state.pred_df = pred_df.copy()
    ...
    if st.session_state.feature_importance is None:
        try:
            ...
            # ✅ bikin jadi persen & rapihin kolom
