...
/* === SIDEBAR INPUT (FIX DROPDOWN & DATE) === */
...
/* === SIDEBAR: bikin teks input/select jadi abu-abu (override global *) === */
section[data-testid="stSidebar"],
section[data-testid="stSidebar"] * {
    color: #fafafa !important;  /* abu-abu */
}

            /* === SELECTBOX SIDEBAR: CUMA TEKS DALAM KOTAK JADI ABU-ABU === */
section[data-testid="stSidebar"] [data-baseweb="select"] div[role="combobox"] * {
    color: #2A2F36 !important;   /* teks abu-abu */
}

/* (Optional tapi recommended) teks placeholder juga */
section[data-testid="stSidebar"] [data-baseweb="select"] div[role="combobox"] {
    color: #2A2F36 !important;
}

            /* ===== SIDEBAR SELECTBOX (Load history, dll): TEKS DI DALAM KOTAK = ABU ===== */
section[data-testid="stSidebar"] div[data-baseweb="select"] > div[role="combobox"] {
    color: #2A2F36 !important;
}

/* semua teks anaknya (value, placeholder, dll) */
section[data-testid="stSidebar"] div[data-baseweb="select"] > div[role="combobox"] * {
    color: #2A2F36 !important;
}

/* kadang value-nya dirender sebagai span/ div tertentu */
    ...
    if st.session_state.pred_df is None:
        ...
        with sub_time:
            ...
            if st.session_state.temporal_importance is not None:
                ...
                # Jika nilainya terlalu uniform, biasanya attention tidak informatif / ada mismatch.
                try:
                    _std = float(temp["Temporal Importance (%)"].std())
                    if _std < 1e-6:
                        st.warning("Temporal importance terlihat uniform (semua hampir sama). Ini biasanya bukan perilaku model, tapi tanda attention tidak informatif / mapping window tidak pas.")
                except Exception:
                    pass
